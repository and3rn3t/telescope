...
const NASA_API_BASE =
  import.meta.env.VITE_NASA_API_BASE || "https://images-api.nasa.gov";
const NASA_API_KEY = import.meta.env.VITE_NASA_API_KEY;
const USE_API_KEY = import.meta.env.VITE_ENABLE_NASA_API_KEY === "true";
const API_TIMEOUT = parseInt(import.meta.env.VITE_NASA_API_TIMEOUT || "10000");
...
function extractDistance(
  ...
  const lightYearMatch = searchText.match(
    /(\d+(?:,\d+)*(?:\.\d+)?)\s*(?:billion|million)?\s*light[- ]?years?/i
  );
  if (lightYearMatch) {
    ...
    let lookbackTime = `${lightYearMatch[1]} ${
  ...
  try {
    ...
    const images: JWSTImage[] = data.collection.items
      ...
      .map((item) => {
        ...
        const distanceData = extractDistance(
          metadata.title,
    ...
    if (error instanceof Error) {
      if (error.name === "AbortError") {
        ...
        throw new Error(
          "NASA API request timed out. Please check your connection."
        );
      } else {
...
export async function testNASAApiKey(): Promise<{
  ...
  try {
    ...
    if (response.ok) {
      ...
      return {
        success: true,
        message: `API key valid! Retrieved APOD: "${
          data[0]?.title || "Untitled"
        }"`,
        apiKeyValid: true,
      };
